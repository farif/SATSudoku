...
public class EfficientEncoding extends EncodingScheme {

	/**
	 * Default Constructor
	 * @param sudoku_
	 */
	public EfficientEncoding(Sudoku sudoku_) {
	...
	/**
	 * Encoding Sudoku
	 * Formula Efficient Encoding = Cell_def AND Cell_uniq  AND Row_uniq AND Col_uniq AND Block_uniq AND Assigned
	 * output: DIMACS
	 */
	public void encodeDIMACS() {

//		System.out.println("Efficient Encoding is used...!");
...
//		int nr_literals = (N * N + N * N * ((N * (N - 1)) / 2)) * 4 + pre_assign_values  * (N + 3 * (N - 1));
		int nr_literals = pre_assign_values  * (N + 3 * (N - 1));
