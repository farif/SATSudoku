...
public class ExtendedEncoding extends EncodingScheme {

	/**
	 * Default Constructor
	 * @param sudoku_
	 */
	public ExtendedEncoding(Sudoku sudoku_) {
...
//		System.out.println("Extended Encoding is used...!");
		...
		// Number of Literals;
		int nr_literals = (N * N + N * N * ((N * (N - 1)) / 2)) * 4
				+ pre_assign_values;
		...
		for (int col = 0; col < N; col++) {
			...
			for (int val = 0; val < N; val++) {
				...
				for (int row = 0; row < N; row++) {
					Literal new_liter = getMappedLiteral(row + 1, col + 1, val + 1,false);
					def_clause.addLiteral(new_liter);
